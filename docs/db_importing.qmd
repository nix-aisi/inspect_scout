...
6. [Transcript API](#transcript-api): Python API for creating and inserting transcripts.

7. [Arrow Import](#arrow-import): Efficient direct insertion using `RecordBatchReader`.

8. [Parquet Data Lake](#parquet-data-lake): Use an existing data lake not created using Inspect Scout.
...
This is the **recommended** way to import OpenClaw data: it reads OpenClaw's own on-disk session files directly, requires no plugins, and captures the full, richest record (including sub-agent topology). Prefer it over [OpenClaw Telemetry](#openclaw-telemetry) unless you only have telemetry captures rather than the native session files.
...
async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(openclaw())
```

By default, all sessions across all agents are imported (scanning `~/.openclaw/agents/*/sessions/`). You can narrow the import by specifying a path, session ID, or time range:

```python
# Import sessions from a specific sessions directory
await db.insert(openclaw(
    path="~/.openclaw/agents/my-agent/sessions",
))
...
OpenClaw transcripts are mapped to the database schema as follows:
...
## OpenClaw Telemetry {#openclaw-telemetry}

Scout can import transcripts from JSONL telemetry files written by the third party [`openclaw-telemetry-hal`](https://github.com/sage-princeton/openclaw-telemetry-hal) plugin (by default to `~/.openclaw/logs/telemetry.jsonl`).

::: {.callout-tip}
## Prefer native sessions

Use this source only when telemetry files are all you have. If you have access to OpenClaw's native session files, use [OpenClaw](#openclaw) (`openclaw()`) instead — it needs no plugin and preserves more of the original record.
:::

Use the `openclaw_telemetry_hal()` transcript source to import a telemetry file.

```python
from inspect_scout import transcripts_db
from inspect_scout.sources import openclaw_telemetry_hal

async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(openclaw_telemetry_hal(
        "~/.openclaw/logs/telemetry.jsonl",
    ))
```

The orchestrator's conversation becomes the transcript's message thread, and each delegated sub-agent is reconstructed as a nested agent span in `events`.

OpenClaw transcripts are mapped to the database schema as follows:

| Field | Value |
|-------|-------|
| `source_type` | `"openclaw_telemetry_hal"` |
| `source_id` | Session/chat id from the OpenClaw session key |
| `transcript_id` | Session/chat id combined with the run's earliest timestamp |
| `agent` | `"openclaw"` |
...
scout import openclaw_telemetry_hal -P path=~/.openclaw/logs/telemetry.jsonl
